...
OverwriteRequest::OverwriteRequest(CpType type, OverwriteCmdType cmd_type)
    : m_cmd_type{cmd_type}, type_{type} {}
...
int OverwriteRequest::do_request() {
  ...
  if (OCT_ENABLE == m_cmd_type) {
    memcpy(buf, "ENABLE_LOG_OVERWRITE ", 21);
    len = 21;
  } else if (OCT_DISABLE == m_cmd_type) {
    memcpy(buf, "DISABLE_LOG_OVERWRITE ", 22);
    len = 22;
  } else {
  // OCT_QUERY == cmd_type
    memcpy(buf, "GET_LOG_OVERWRITE ", 18);
    len = 18;
  }
