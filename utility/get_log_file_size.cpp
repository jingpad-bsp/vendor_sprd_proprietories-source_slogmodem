...
GetLogFileSize::GetLogFileSize(CpType type)
    : type_{type} {}
...
int GetLogFileSize::do_request() {
  ...
  memcpy(buf, "GET_LOG_FILE_SIZE ", 18);
  ...
  if (err || (rlen == tlen)) {
    fprintf(stderr,
            "Can not make the subsystem parameter of GET_LOG_FILE_SIZE\n");
    return -1;
  }

  p += tlen;
  *p = '\n';
  err = send_req(buf, tlen + 19);
