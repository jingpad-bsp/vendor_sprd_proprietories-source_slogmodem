...
int AgDspLogHandler::start(LogConfig::LogMode /*mode*/) {
...
void AgDspLogHandler::add_headers(uint8_t* buf, size_t len, DspLogType type) {
  ...
  buf[8] = 0x5a;
  buf[9] = 0x5a;
  // TCP Checksum (only cover LEN, CHANNEL, TYPE and RESERVED)
...
void AgDspLogHandler::dump_result_notifiy(void* hdl, AgdspDump::Dump_State ds) {
  ...
  agdsp_hdl->notify_dump_end();
}

int AgDspLogHandler::save_audio_parameter() {
  int ret = -1;

  DIR* pd = opendir(AUDIO_PARAM_FILE_PATH);

  if (!pd) {
    err_log("fail to open %s", AUDIO_PARAM_FILE_PATH);
    return ret;
  }

  LogVector<LogString> file_list;
  struct dirent* dent;
  ...
  while (true) {
    ...
    LogString file_path
        = LogString(AUDIO_PARAM_FILE_PATH) + "/" + dent->d_name;
  ...
  if (file_list.size()) {
    if (LCR_SUCCESS == copy_files(LogFile::LT_UNKNOWN, file_list)) {
      ret = 0;
    }
  } else {
    info_log("no audio param files are available");
    ret = 0;
  }
