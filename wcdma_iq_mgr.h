...
class WcdmaIqManager : public FdHandler {
 ...
 private:
  ...
  LogList<std::weak_ptr<LogFile>> m_iq_files;
  // I/Q buffer mapped
  uint8_t* m_iq_buffer;
  // I/Q buffer size
  uint32_t m_iq_memsize;
  // File for current I/Q segment
  std::weak_ptr<LogFile> m_cur_seg_file;
  // Notification receiver for I/Q storage thread
  SaveIqNotifier* m_iq_stor_notifier;
  // I/Q storage thread
  pthread_t m_iq_stor_thread;
  // Mutex between the main thread and the I/O thread
  ...
  std::weak_ptr<LogFile> create_seg_file();
  ...
  /*  notify_stor_inactive - callback when storage inactive.
   */
