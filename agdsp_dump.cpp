...
void AgdspDump::start_dump(CpStorage& stor, const LogString& name) {
  ...
  LogController::save_sipc_dump(&stor, lt);

  pthread_mutex_lock(&m_lock);

  char td[32];
  snprintf(td, sizeof td,
           "%04d%02d%02d-%02d%02d%02d.mem",
           lt.tm_year + 1900,
           lt.tm_mon + 1,
           lt.tm_mday,
           lt.tm_hour,
...
void AgdspDump::process_stor_response(int resp) {
  if (DS_DUMPING == m_state) {
    ...
    if (DumpStoreNotifier::DUMP_SAVED == resp) {
      m_state = DS_SUCCESS;
      if (dump_file) {
        dump_file->count_size(m_saved_size);
        dump_file->close();
      }
...
int AgdspDump::save_dump() {
  ...
  while (true && dump_file) {
    ...
    ssize_t nwr = dump_file->write_raw(buf, nr);
    if (nwr != nr) {
