...
class AgDspLogHandler : public LogPipeHandler {
 public:
  ...
  AgDspLogHandler(const AgDspLogHandler&) = delete;

  ~AgDspLogHandler();

  AgDspLogHandler& operator = (const AgDspLogHandler&) = delete;
  ...
  /*
   *    process_assert - Process the agdsp assertion.
   *
