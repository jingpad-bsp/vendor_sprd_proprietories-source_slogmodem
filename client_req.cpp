...
int send_response(int conn, ResponseErrorCode err) {
  ...
  if (static_cast<size_t>(n) == len) {
    ...
    err_log("resp = %s, ret = %d", resp, ret);
  }

  return ret;
}

int send_text_response(int conn, const char* dest_str) {
  char resp[32];
  ...
  switch (len) {
    ...
    case 5:
      if (!memcmp(cp, "WCDMA", 5)) {
        ...
        t = CT_5MODE;
      } else if (!memcmp(cp, "PM_SH", 5)) {
        t = CT_PM_SH;
      }
      break;
    case 6:
      if (!memcmp(cp, "ORCAAP", 6)) {
        t = CT_ORCA_MINIAP;
      } else if (!memcmp(cp, "ORCADP", 6)) {
        t = CT_ORCA_DP;
      }
#ifdef SUPPORT_AGDSP
      else if (!memcmp(cp, "AG-DSP", 6)) {
        t = CT_AGDSP;
      }
#endif
      break;
...
LogFile::LogType get_file_type(const uint8_t* type, size_t len) {
  LogFile::LogType t = LogFile::LT_UNKNOWN;

  switch (len) {
    case 4:
      if (!memcmp(type, "DUMP", 4)) {
        t = LogFile::LT_DUMP;
      }
      break;
    default:
