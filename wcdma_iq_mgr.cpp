...
int WcdmaIqManager::start(CpStorage* cp_stor) {
  ...
  m_state = IQS_WAITING;
  m_cp_stor->subscribe_stor_inactive_evt(this, notify_stor_inactive);
  return 0;
...
int WcdmaIqManager::stop() {
  if (IQS_IDLE != m_state) {
    m_cp_stor->unsubscribe_stor_inactive_evt(this);
    ...
    if (auto cur_seg_file = m_cur_seg_file.lock()) {
      cur_seg_file->count_size(m_saved_size);
      cur_seg_file->close();
      m_cur_seg_file.reset();
    }
    ...
    munmap(m_iq_buffer, m_iq_memsize);
    close();
...
int WcdmaIqManager::prepare_iq_mem_map() {
  int iq_memsize_fd;
  char csize[IQ_SIZE_LENGTH + 1] = {0};
  char* endp;
  unsigned long iq_memsize = ULONG_MAX;
  ...
  // read iq mem size
  iq_memsize_fd = open(IQ_MEM_SIZE, O_RDONLY);
  if (iq_memsize_fd < 0) {
    err_log("open %s failed", IQ_MEM_SIZE);
    close();
    return -1;
  }
  err = read(iq_memsize_fd, csize, IQ_SIZE_LENGTH);
  ::close(iq_memsize_fd);
  if (err < 0) {
    err_log("read %s failed", IQ_MEM_SIZE);
    close();
    return -1;
  }
  info_log("read %s: %s", IQ_MEM_SIZE, csize);
  iq_memsize = strtoul(csize, &endp, 10);
  if (!iq_memsize || (ULONG_MAX == iq_memsize && ERANGE == errno)) {
    err_log("csize strtoul failed");
    close();
    return -1;
  }
  info_log("iq_memsize %#lx", iq_memsize);
  m_iq_memsize = static_cast<uint32_t>(iq_memsize);

  // Map the buffer
  m_iq_buffer = static_cast<uint8_t*>(
      mmap(0, m_iq_memsize, PROT_READ | PROT_WRITE, MAP_PRIVATE, m_fd, 0));
...
int WcdmaIqManager::gen_iq_file_name(LogString& file_name) const {
  ...
  snprintf(time_str, sizeof time_str, "_wiq_%04d%02d%02d-%02d%02d%02d.bin",
...
std::weak_ptr<LogFile> WcdmaIqManager::create_seg_file() {
  while (m_iq_files.size() >= m_max_iq_segs) {
    auto it = m_iq_files.begin();
    if (it == m_iq_files.end()) {
      break;
    }

    std::weak_ptr<LogFile> f = *it;
    m_iq_files.erase(it);
    // check if the iq log still exist
    if (auto iq_file = f.lock()) {
      iq_file->dir()->remove(iq_file);
    }
  }
  ...
  if (0 == gen_iq_file_name(iq_file_name)) {
    f = m_cp_stor->create_file(iq_file_name, LogFile::LT_WCDMA_IQ);
  }
...
int WcdmaIqManager::start_saving_thread() {
  ...
  multiplexer()->register_fd(m_iq_stor_notifier, POLLIN);
...
void WcdmaIqManager::process(int /*events*/) {
  ...
  auto cur_seg_file = m_cur_seg_file.lock();
  if (!cur_seg_file) {
    ioctl(fd(), CMD_SET_IQ_RD_FINISHED);
  ...
  if (m_iq_stor_notifier->send_command(SaveIqNotifier::IQST_SAVE)) {
    ...
    cur_seg_file->close();
    cur_seg_file->dir()->remove(cur_seg_file);
    m_cur_seg_file.reset();
    ioctl(fd(), CMD_SET_IQ_RD_FINISHED);
  } else {
    m_iq_files.push_back(m_cur_seg_file);
    m_state = IQS_SAVING;
...
bool WcdmaIqManager::process_save() {
  ...
  while (remain_len) {
    ...
    ssize_t n = cur_seg_file->write_raw(p, block_len);
    if (n > 0) {
      remain_len -= n;
      p += n;
    }
...
void WcdmaIqManager::notify_stor_inactive(void *client, unsigned priority) {
  WcdmaIqManager* wiq = static_cast<WcdmaIqManager*>(client);

  if (IQS_SAVING == wiq->m_state) {
    auto cur_seg_file = wiq->m_cur_seg_file.lock();

    if (cur_seg_file &&
        priority == cur_seg_file->dir()->cp_set_dir()->priority()) {
      wiq->clear_iq_files();
    }
  }
}
