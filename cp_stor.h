...
class CpStorage {
 public:
  using new_log_callback_t = void (*)(LogPipeHandler*, LogFile* f);

  CpStorage(StorageManager& stor_mgr, LogPipeHandler& cp);
  CpStorage(const CpStorage&) = delete;
  ~CpStorage();

  CpStorage& operator = (const CpStorage&) = delete;

  bool current_file_saving() const { return !m_cur_file.expired(); }
  ...
  // Non-log files
  std::weak_ptr<LogFile> create_file(const LogString& name, LogFile::LogType t,
                                     StorageManager::MediaType mt =
                                         StorageManager::MT_STOR_END);
   ...
   */
  void subscribe_stor_inactive_evt(void* client,
                                   StorageManager::stor_umount_callback_t cb);

  /*  unsubscribe_stor_inactive_evt - unsubscribe the service of storage umount.
   *  @client_ptr - non log file clients:
   *                sleep log/ringbuffer/minidump/memory dump
   */
  void unsubscribe_stor_inactive_evt(void* client);

  /*  check_quota - check quota.
   *  @mt: the media whose quota is to be checked. If mt is
   *       StorageManager::MT_STOR_END, check quota for the current media.
   *
   *  Return Value:
   *    Return 0 if there is more storage capacity, -1 otherwise.
   */
  int check_quota(StorageManager::MediaType mt = StorageManager::MT_STOR_END);

  /*  on_cur_media_disabled - called when current media is disabled
   *                          administratively.
   *
   *  This function will flush buffered log and close current log file.
   */
  void on_cur_media_disabled();

 private:
  /*  on_file_size_update - check quota.
   *  @lf:  current saving file
   ...
   *           if necessary.
   */
  void on_file_size_update(LogFile* lf, int err);

  static void file_wr_callback(void* client, LogFile* lf, int err);
 ...
 private:
  ...
  // Current log file
  std::weak_ptr<LogFile> m_cur_file;
  // Log capacity state
  bool m_shall_stop;
